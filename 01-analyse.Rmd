...
```{r group-by-site-state, echo=FALSE}
my_packages <- c("tidyverse", "knitr", "kableExtra", "anytime")
temp <- lapply(my_packages, library, character.only = TRUE)

info_tbl <- readRDS("data-processed/sites.rds")

sites_state <- info_tbl[-c(1, 3:25, 27:33)]
...
sites <- info_tbl[-c(1, 3:22, 24:25, 27:33)]
sites <- sites[order(sites$total_users, decreasing = TRUE), ]
sites <- rownames_to_column(sites, var = "#")
sites$`#` <- as.numeric(sites$`#`)

# add marker if website is still Beta
sites <- sites %>% as_tibble() %>% mutate(
    site_state_date = if_else(
        is.na(info_tbl$site_launch_date), # check if NA
        paste(as.character(anydate(info_tbl$site_open_beta_date)), "B"), # replace with beta date
        as.character(anydate(info_tbl$site_launch_date)) # not NA then normal launch date
    )
)
...
knitr::kable(sites, 
        ...
        longtable = T,
        row.names = F,
        col.names = c("#*", "Name", "Users", "Launched†", "URL"),
        caption = "Full List of Websites of the StockExchange Network (May 25th, 2018") %>%
        ...
        footnote(symbol = c(
            ...
            "B = launch date of open beta, e.g. site is still in beta")
            )
